  ...
  if (atom(e)) {
    ...
    switch (car(e)) {
      ...
      case LET:
        // let (bindings) expr
        // car cadr       caddr
        return lEval(caddr(e), bind(cadr(e), a))
      case LET_STAR:
        return lEval(caddr(e), bindSeq(cadr(e), a))
      default:
...
const bind = (bindings, a) => {
  const binding = car(bindings)
  if (binding === NIL) return a
  const boundContext = bind(cdr(bindings), a)
  if (atom(binding)) {
    return cons(cons(binding, NIL), boundContext)
  } else {
    const symbol = car(binding)
    const value = lEval(cadr(binding), a)
    return cons(cons(symbol, value), boundContext)
  }
}

const bindSeq = (bindings, a) => {
  const binding = car(bindings)
  if (binding === NIL) return a
  return bindSeq(cdr(bindings), bind(cons(binding, NIL), a))
}
...
export const LET_STAR = Symbol('let*')
